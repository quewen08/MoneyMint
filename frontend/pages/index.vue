<template>
  <div>
    <!-- 页面标题 -->
    <h1 class="text-2xl font-bold mb-6">首页</h1>

    <!-- 错误信息显示 -->
    <div
      v-if="ledger.errors && ledger.errors.length > 0"
      class="card bg-red-50 border border-red-200 mb-6"
    >
      <h2 class="text-xl font-semibold mb-3 text-red-800">账本错误</h2>
      <div class="space-y-3 max-h-60 overflow-y-auto">
        <div
          v-for="(error, index) in ledger.errors"
          :key="index"
          class="bg-white p-3 rounded shadow-sm border-l-4 border-red-500"
        >
          <div class="flex justify-between items-start mb-1">
            <span class="font-medium text-red-700">错误 {{ index + 1 }}</span>
            <span class="text-xs bg-red-100 text-red-800 px-2 py-1 rounded">{{
              error.severity || "Error"
            }}</span>
          </div>
          <p class="text-sm text-gray-700 mb-1">{{ error.message }}</p>
          <div v-if="error.source" class="text-xs text-gray-500">
    ...
    <div class="mx-auto py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="card bg-gradient-to-br from-red-50 to-red-100 p-6">
          <div class="flex justify-between items-center">
            <div>
              <p class="text-sm text-red-700 font-medium">总支出</p>
        ...
        <div class="card bg-gradient-to-br from-green-50 to-green-100 p-6">
          <div class="flex justify-between items-center">
            <div>
              <p class="text-sm text-green-700 font-medium">净收入</p>
      ...
      <!-- 分类统计图表（简单版） -->
      <div class="card mb-8">
        <h2 class="text-xl font-semibold mb-4">分类支出</h2>
        <div class="space-y-3">
          <div
...
const { getLedger, getEntries, user } = useApi();
