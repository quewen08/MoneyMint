  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div class="card p-4 bg-blue-50">
        ...
        <div class="text-2xl font-bold text-blue-900">{{ formatCurrency(totalAssets) }}</div>
      </div>
      <div class="card p-4 bg-red-50">
        <div class="text-sm text-red-700 mb-1">总负债</div>
        <div class="text-2xl font-bold text-red-900">{{ formatCurrency(totalLiabilities) }}</div>
      </div>
      <div class="card p-4 bg-green-50">
        <div class="text-sm text-green-700 mb-1">净资产</div>
        <div class="text-2xl font-bold text-green-900">{{ formatCurrency(totalNetWorth) }}</div>
      </div>
    </div>
          ...
          <div class="space-y-2">
            ...
            >
              <div class="flex items-center">
                <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3" :class="getAccountTypeColor(type)">
              ...
              <div class="text-right">
                <div class="font-medium" :class="account.balance >= 0 ? 'text-green-600' : 'text-red-600'">
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            ...
            <button @click="selectedAccount = null" class="text-gray-500 hover:text-gray-700">
          ...
          <div class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div class="font-medium">{{ getAccountTypeName(selectedAccount.type) }}</div>
              </div>
              <div>
                <div class="text-sm text-gray-500 mb-1">账户说明</div>
                <div class="font-medium">{{ selectedAccount.note || '无' }}</div>
              </div>
              <div>
              ...
              <div class="col-span-2">
                ...
                <div class="text-2xl font-bold" :class="selectedAccount.balance >= 0 ? 'text-green-600' : 'text-red-600'">
        ...
        <div class="p-6">
          ...
          <form @submit.prevent="handleAddAccount">
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">账户说明</label>
                <input 
              ...
              <div class="pt-4">
                <button 
                  ...
                  :disabled="submitting"
                >
                  <span v-if="submitting" class="inline-block animate-spin rounded-full h-4 w-4 mr-2 border-b-2 border-white"></span>
                  新增账户
                </button>
              </div>
            </div>
    ...
    <!-- 关闭账户弹窗 -->
    <div v-if="showCloseAccountModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold">关闭账户</h3>
            <button @click="showCloseAccountModal = false" class="text-gray-500 hover:text-gray-700">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <div class="space-y-4">
            <div class="text-sm text-gray-600">
              确定要关闭账户 <span class="font-medium">{{ selectedAccount?.name }}</span> 吗？
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">日期</label>
              <input 
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="p-6">
          ...
          <form @submit.prevent="handleSetBalance">
            <div class="space-y-4">
              <div class="text-sm text-gray-600">
                为账户 <span class="font-medium">{{ selectedAccount?.name }}</span> 设置余额
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">日期</label>
                <input 
...
const getAccountTypeName = (type: string): string => {
  switch (type) {
    case "Assets": return "资产账户";
    case "Liabilities": return "负债账户";
    case "Equity": return "权益账户";
    case "Income": return "收入账户";
    case "Expenses": return "支出账户";
    default: return type;
  }
};
...
const getAccountTypeColor = (type: string): string => {
  switch (type) {
    ...
    case "Liabilities": return "bg-red-500";
...
const totalNetWorth = computed(() => totalAssets.value - totalLiabilities.value);
...
// 加载账户数据
const loadAccounts = async (dateRangeParams: {start_date?: string, end_date?: string} = {}) => {
  try {
    loading.value = true;
